...
try:
    ...
    print(f'Sorry, we don’t have the birthday information for {a_name}')
...
a_birthday = ''
while True:
    a_birthday = input('Give his birthday! (YYYY/MM/DD) \t')
    if not re.match(pattern, a_birthday.strip()):
        continue
    else:
        break
# Now add this new data into your dictionary.
if a_name in birthdays.keys():
    a_name = f'{a_name} +'
birthdays[a_name] = a_birthday
...
a_name = input('Give a name to display a birthday.\t')
try:
    print(f'{a_name}\'s birthday is {birthdays[a_name]}')
except:
    print(f'Sorry, we don’t have the birthday information for {a_name}')
...
for key, value in items.items():
    sentence += f'{key.capitalize()} is {value}$. '
print(sentence)
